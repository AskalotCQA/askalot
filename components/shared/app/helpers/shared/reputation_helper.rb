module Shared::ReputationHelper
  def reputation_icon(user)
    ...
    return reputation_icon_map[:gold] if rank < Shared::Configuration.reputation.gold
    return reputation_icon_map[:silver] if rank < Shared::Configuration.reputation.silver
  ...
  def reputation_ranking(reputation)
    Shared::User::Profile.of('reputation').where('value > ?', reputation).count / Shared::User::Profile.of('reputation').where('value > 0').count.to_f * 100
  end
end
