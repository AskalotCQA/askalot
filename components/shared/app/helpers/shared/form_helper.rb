module Shared::FormHelper
  ...
  def category_collection_select(id, collection = Shared::Category.askable.includes(:assignments).order(:name), value = :id, label = :full_public_name, options = {}, html_options = {})
    ...
    return grouped_collection_select(id, collection, :leaves_with_metadata, label, ->(el){el.first[value]}, ->(el){el.first[label]}, options, html_options) if options[:grouped]
    collection_select(id, collection, value, label, options, html_options) unless options[:grouped]
  end
