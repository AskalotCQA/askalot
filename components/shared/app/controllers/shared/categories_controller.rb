...
class CategoriesController < ApplicationController
  ...
  def index
    @context_categories = Shared::Category.in_contexts(@context_id).visible.not_unknown.includes(:assignments, :watchers).order('full_tree_name')
    category_ids        = Shared::Category.in_contexts(@context_id).visible.not_unknown.pluck(:id)
