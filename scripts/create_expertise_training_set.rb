...
def answers_in_category(user, resource, category)
  return ExtractFeatures.user_answers_older_than_resource(resource, user).in_context(category).count
end

def votes_in_category(user, resource, category)
  return ExtractFeatures.user_answers_older_than_resource(resource, user).in_context(category).sum(:votes_difference)
end
...
def build_features(resource, category, question, user, class_id)
  ...
  knowledge_gap_f = knowledge_gap(user, question.author, resource, category)
  ...
  if cos_sim_f.to_f > 0
    $output_file << "#{class_id} #{cos_sim_f} #{answers_in_week_f} #{answers_in_topic_f} #{votes_in_topic_f} #{votes_in_week_f} "\
                    "#{asker_knowledge_f} #{answerer_knowledge_f} #{knowledge_gap_f} "\
                    "#{seen_week_categories_f} #{seen_topic_categories_f}\n"
  end
end
...
answers.each do |answer|
  ...
  unless ExtractFeatures.is_student(answer.author)
